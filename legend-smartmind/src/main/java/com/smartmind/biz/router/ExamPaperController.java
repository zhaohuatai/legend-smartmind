...
@Controller 
@RequestMapping("/manage/smartmind/exampaper")
public class ExamPaperController implements BaseController {

	
	@Autowired
	private IExamPaperService examPaperService;
	
	//@RequiresPermissions(value="examPaper:loadDataSet",desc="试卷列表")
    @RequestMapping(value="/loadDataSet", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody  Object loadDataSet(@RequestBody ExamPaperPageQueryDto pageQueryDto){
    	 ChainWrapper<ExamPaper> chainWrapper = DtoToWrapperUtil.convertToChainWrapper(pageQueryDto, ExamPaper.class);
	     DataSetDto<ExamPaper> dataSet=  examPaperService.loadDataSetByQuery(chainWrapper);
	    return ajaxQueryResult( dataSet);
    }
	
	//@RequiresPermissions(value="examPaper:create",desc="试卷添加")
    @RequestMapping(value="/create", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public  @ResponseBody Object create(@RequestBody ExamPaperCreateDto examPaperCreateDto){
		examPaperService.createExamPaper(examPaperCreateDto,SecurityUtil.getSimpleUser());
    	return ajaxDoneSuccess("数据操作成功");
    }
	
	//@RequiresPermissions(value="examPaper:load",desc="试卷加载")
    ...
    public @ResponseBody  Object loadById(@PathVariable Long id){
		Optional<ExamPaper> ops = examPaperService.loadById(id);
		return ajaxQueryResult(ops);
    }
	
	//@RequiresPermissions(value="examPaper:update",desc="试卷更新")
    @RequestMapping(value="/update", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody  Object update(@RequestBody ExamPaperUpdateDto examPaperUpdateDto){
		examPaperService.updateExamPaper(examPaperUpdateDto,SecurityUtil.getSimpleUser());
		return ajaxDoneSuccess("数据操作成功");
    }
	
	//@RequiresPermissions(value="examPaper:delete",desc="试卷删除")
    ...
    @RequiresPermissions(value="examPaper:load",desc="宠物信息加载")
    ...
    public @ResponseBody  Object setStatus(@RequestBody StatusDto status){
		examPaperService.setStatus(status);
		return ajaxDoneSuccess("数据操作成功");
    }
	...
	//	List<SelectVo>  ops = examPaperService.loadSelectVo(SecurityUtil.getTenantId(),"1",keywords);
