...
@Controller 
@RequestMapping("/manage/smartmind/teachingfile")
public class TeachingFileController implements BaseController {

	
	@Autowired
	private ITeachingFileService teachingFileService;
	
	//@RequiresPermissions(value="teachingFile:loadDataSet",desc="教学文件列表")
    @RequestMapping(value="/loadDataSet", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody  Object loadDataSet(@RequestBody TeachingFilePageQueryDto pageQueryDto){
    	 ChainWrapper<TeachingFile> chainWrapper = DtoToWrapperUtil.convertToChainWrapper(pageQueryDto, TeachingFile.class);
	     DataSetDto<TeachingFile> dataSet=  teachingFileService.loadDataSetByQuery(chainWrapper);
	    return ajaxQueryResult( dataSet);
    }
	
	//@RequiresPermissions(value="teachingFile:create",desc="教学文件添加")
    @RequestMapping(value="/create", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public  @ResponseBody Object create(@RequestBody TeachingFileCreateDto teachingFileCreateDto){
		teachingFileService.createTeachingFile(teachingFileCreateDto,SecurityUtil.getSimpleUser());
    	return ajaxDoneSuccess("数据操作成功");
    }
	
	//@RequiresPermissions(value="teachingFile:load",desc="教学文件加载")
    ...
    public @ResponseBody  Object loadById(@PathVariable Long id){
		Optional<TeachingFile> ops = teachingFileService.loadById(id);
		return ajaxQueryResult(ops);
    }
	
	//@RequiresPermissions(value="teachingFile:update",desc="教学文件更新")
    @RequestMapping(value="/update", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody  Object update(@RequestBody TeachingFileUpdateDto teachingFileUpdateDto){
		teachingFileService.updateTeachingFile(teachingFileUpdateDto,SecurityUtil.getSimpleUser());
		return ajaxDoneSuccess("数据操作成功");
    }
	
	//@RequiresPermissions(value="teachingFile:delete",desc="教学文件删除")
    //@RequestMapping(value="/delete", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    //public @ResponseBody  Object delete(Long id){
	//	teachingFileService.delete(id);
	//	return ajaxDoneSuccess("数据操作成功");
    //}
    
    @RequiresPermissions(value="teachingFile:load",desc="宠物信息加载")
    ...
    public @ResponseBody  Object setStatus(@RequestBody StatusDto status){
		teachingFileService.setStatus(status);
		return ajaxDoneSuccess("数据操作成功");
    }
	...
	//	List<SelectVo>  ops = teachingFileService.loadSelectVo(SecurityUtil.getTenantId(),"1",keywords);
