      ...
      <template v-for="item in menuTree" :key="item.id">
        <template v-if="!item.hidden">
          <a-menu-item :key="item.id.toString()" @click="onSelectMenu(item)">
            ...
            {{ menuNameAdapter(item.meta?.title || item.name) }}
          </a-menu-item>
        </template>
      </template>
    </a-menu>
...
<script setup>
  ...
  function onSelectMenu(menuItem) {
    ...
    if (menuItem.menuType === MENU_TYPE_ENUM.MENU.value && (_.isEmpty(menuItem.children) || menuItem.children.every((e) => e.hide === 1))) {
