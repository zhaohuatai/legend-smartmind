    ...
    <a-dropdown :trigger="['contextmenu']">
      <div class="smart-page-tag">
        <a-tabs style="width: 100%" :tab-position="mode" v-model:activeKey="selectedKey" size="small" @tabClick="selectTab">
          <a-tab-pane v-for="item in tagNav" :key="item.name">
            <template #tab>
              <span class="smart-page-tag-content">
                <home-outlined style="font-size: 12px" v-if="item.name === HOME_PAGE_NAME" class="smart-page-tag-close" />
                <component class="smart-page-tag-icon" v-else :is="$antIcons[item.icon]" />
                {{ item.title }}
                <close-outlined @click.stop="closeTag(item, false)" v-if="item.name !== HOME_PAGE_NAME" class="smart-page-tag-close" />
              </span>
            </template>
          </a-tab-pane>
        </a-tabs>
...
<script setup>
  ...
  function selectTab(name) {
    ...
    // 寻找tag
    let tag = tagNav.value.find((e) => e.name === name);
    if (!tag) {
      router.push({ name: HOME_PAGE_NAME });
      return;
    }
    // router.push({ name, query: Object.assign({ _keepAlive: 1 }, tag.query) });
    router.push({ name, query: tag.query });
  }

  //通过菜单关闭
  function closeByMenu(closeAll) {
    let find = tagNav.value.find((e) => e.name === selectedKey.value);
    if (!find || closeAll) {
      closeTag(null, true);
    } else {
      closeTag(find, true);
  ...
  function closeTag(item, closeAll) {
    ...
    if (item && !closeAll) {
      ...
      if (item.fromName && item.fromName !== item.name && tagNav.value.some((e) => e.name === item.fromName)) {
        goName = item.fromName;
        goQuery = item.fromQuery;
      } else {
        // 查询左侧tag
        let index = tagNav.value.findIndex((e) => e.name === item.name);
        if (index > 0) {
          // 查询左侧tag
          let leftTagNav = tagNav.value[index - 1];
          goName = leftTagNav.name;
          goQuery = leftTagNav.query;
        }
      }
    ...
    useUserStore().closeTagNav(item ? item.name : null, closeAll);
  }
