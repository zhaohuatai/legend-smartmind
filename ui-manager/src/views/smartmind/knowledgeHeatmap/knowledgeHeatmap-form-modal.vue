<template>
  <a-modal
    :open="visible"
    :title="form.id ? '编辑知识点热力图' : '新建知识点热力图'"
    ok-text="确认"
    cancel-text="取消"
    @ok="onSubmit"
    @cancel="onClose"
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
      ...
      <a-row>
        ...
        <a-col :span="8">
          <a-form-item label="知识点标签" name="knowledgeTag">
            <a-input v-model:value="form.knowledgeTag" placeholder="请输入知识点标签"  />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row>
...
<script setup>
  ...
  const rules = {
    ...
    knowledgeTag: [{ required: true, message: '请输入知识点标签' }],
  ...
  async function onSubmit() {
    try {
      ...
      if (form.id) {
        await knowledgeHeatmapApi.update(form);
        message.success('修改成功');
      } else {
        await knowledgeHeatmapApi.add(form);
