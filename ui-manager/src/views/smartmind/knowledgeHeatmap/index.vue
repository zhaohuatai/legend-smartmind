  ...
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        ...
        <a-form-item label="知识点标签" class="smart-query-form-item">
          <a-input style="width: 200px" v-model:value="queryForm.knowledgeTag" placeholder="请输入知识点标签" />
        </a-form-item>
    ...
    <KnowledgeHeatmapFormModal ref="formModalRef" @reloadList="queryData" />
  </div>
</template>
...
<script setup>
  ...
  import { knowledgeHeatmapApi } from '/@/api/smartmind/knowledgeHeatmap-api.js';
  import { PAGE_SIZE_OPTIONS } from '/@/constants/common-const';
  import { smartSentry } from '/@/lib/smart-sentry';
  import { useDictStore } from '/@/store/modules/system/dict.js';
  import KnowledgeHeatmapFormModal from './knowledgeHeatmap-form-modal.vue';
  ...
  async function queryData() {
    try {
      ...
      let res = await knowledgeHeatmapApi.queryPage(queryForm);
  ...
  async function toDelete(record) {
    try {
      await knowledgeHeatmapApi.delete(record.id);
