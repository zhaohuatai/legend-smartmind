  ...
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="签到活动ID" class="smart-query-form-item">
          <a-input style="width: 200px" v-model:value="queryForm.signId" placeholder="请输入签到活动ID" />
        </a-form-item>
        ...
        <a-form-item label="签到时间" class="smart-query-form-item">
          <a-date-picker v-model:value="queryForm.signTime" placeholder="请选择签到时间" style="width: 200px" />
        </a-form-item>
      ...
      <a-table size="small" :loading="tableLoading" bordered :dataSource="tableData" :columns="columns" rowKey="id" :pagination="false">
        <template #bodyCell="{ record, column, index }">
          ...
          <template v-else-if="column.dataIndex === 'signResult'">
            {{ dictStore.getDataLabels('sign_result', record.signResult) }}
          </template>
          <template v-else-if="column.dataIndex === 'signMethod'">
            {{ dictStore.getDataLabels('sign_method', record.signMethod) }}
          </template>
          <template v-else-if="column.dataIndex === 'leaveEarly'">
            {{ dictStore.getDataLabels('yes_no', record.leaveEarly) }}
          </template>
    ...
    <SignInRecordFormModal ref="formModalRef" @reloadList="queryData" />
  </div>
</template>
...
<script setup>
  ...
  import SignInRecordFormModal from './signInRecord-form-modal.vue';
  ...
  const columns = ref([
    ...
    {
      title: '签到结果: 1-正常, 2-迟到, 3-缺勤, 4-请假',
      dataIndex: 'signResult',
      width: 120,
      ellipsis: true,
    },
    {
      title: '签到方式: 1-点击签到, 2-位置签到, 3-手势签到, 4-扫码签到',
  ...
  async function queryData() {
    try {
      ...
      let res = await signInRecordApi.queryPage(queryForm);
  ...
  async function toDelete(record) {
    try {
      await signInRecordApi.delete(record.id);
