<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
      <a-row>
        <a-col :span="8">
          <a-form-item label="签到编码" name="signCode">
            <a-input v-model:value="form.signCode" placeholder="请输入签到编码"  />
          </a-form-item>
        </a-col>
        <a-col :span="8">
        ...
        <a-col :span="8">
          <a-form-item label="签到类型: 1-普通签到, 2-位置签到, 3-手势签到, 4-扫码签到" name="signType">
            <a-select v-model:value="form.signType" placeholder="请选择签到类型: 1-普通签到, 2-位置签到, 3-手势签到, 4-扫码签到" allowClear>
              <a-select-option v-for="item in dictStore.getDictData('sign_type')" :key="item.dataValue" :value="item.dataValue">{{ item.dataLabel }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-row>
        ...
        <a-col :span="8">
          <a-form-item label="签到状态: 0-未开始, 1-进行中, 2-已结束, 3-已取消" name="signStatus">
            <a-select v-model:value="form.signStatus" placeholder="请选择签到状态: 0-未开始, 1-进行中, 2-已结束, 3-已取消" allowClear>
              <a-select-option v-for="item in dictStore.getDictData('sign_status')" :key="item.dataValue" :value="item.dataValue">{{ item.dataLabel }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-row>
        ...
        <a-col :span="8">
          <a-form-item label="签到配置(如手势密码、位置范围等)" name="signConfig">
            <a-textarea v-model:value="form.signConfig" :rows="3" placeholder="请输入签到配置(如手势密码、位置范围等)" />
          </a-form-item>
        </a-col>
      </a-row>
    </a-form>
...
<script setup>
  ...
  const rules = {
    signCode: [{ required: true, message: '请输入签到编码' }],
    signName: [{ required: true, message: '请输入签到名称' }],
    signType: [{ required: true, message: '请输入签到类型: 1-普通签到, 2-位置签到, 3-手势签到, 4-扫码签到' }],
    ...
    signStatus: [{ required: true, message: '请输入签到状态: 0-未开始, 1-进行中, 2-已结束, 3-已取消' }],
  ...
  async function onSubmit() {
    try {
      ...
      if (form.id) {
        await signInActivityApi.update(form);
        message.success('修改成功');
      } else {
        await signInActivityApi.add(form);
