<template>
  <div>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-table size="small" :loading="tableLoading" bordered :dataSource="tableData" :columns="columns" rowKey="id" :pagination="false">
        <template #bodyCell="{ record, column, index }">
          ...
          <template v-else-if="column.dataIndex === 'arrangementType'">
            {{ dictStore.getDataLabels('arrangement_type', record.arrangementType) }}
          </template>
    ...
    <ClassCourseFormModal ref="formModalRef" @reloadList="queryData" />
  </div>
</template>
...
<script setup>
  ...
  import ClassCourseFormModal from './classCourse-form-modal.vue';
  ...
  const columns = ref([
    ...
    {
      title: '安排类型: 1-固定班级, 2-走班排课',
      dataIndex: 'arrangementType',
      width: 120,
      ellipsis: true,
    },
    {
      title: '状态: 0-失效, 1-有效, 2-已结课',
      dataIndex: 'status',
      width: 56,
      ellipsis: true,
    },
  ...
  async function queryData() {
    try {
      ...
      let res = await classCourseApi.queryPage(queryForm);
  ...
  async function toDelete(record) {
    try {
      await classCourseApi.delete(record.id);
