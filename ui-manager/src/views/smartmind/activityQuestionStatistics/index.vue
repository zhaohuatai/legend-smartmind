  ...
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        ...
        <a-form-item label="错误类型" class="smart-query-form-item">
          <a-input style="width: 200px" v-model:value="queryForm.commonMistakeType" placeholder="请输入错误类型" />
        </a-form-item>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-table size="small" :loading="tableLoading" bordered :dataSource="tableData" :columns="columns" rowKey="id" :pagination="false">
        <template #bodyCell="{ record, column, index }">
          ...
          <template v-else-if="column.dataIndex === 'optionChanged'">
            {{ dictStore.getDataLabels('yes_no', record.optionChanged) }}
          </template>
    ...
    <ActivityQuestionStatisticsFormModal ref="formModalRef" @reloadList="queryData" />
  </div>
</template>
...
<script setup>
  ...
  import ActivityQuestionStatisticsFormModal from './activityQuestionStatistics-form-modal.vue';
  ...
  const columns = ref([
    ...
    {
      title: '是否修改过答案: 0-否, 1-是',
      dataIndex: 'optionChanged',
      width: 120,
      ellipsis: true,
    },
    {
      title: '错误类型',
      dataIndex: 'commonMistakeType',
      ellipsis: true,
    },
    {
      title: '统计时间',
  ...
  async function queryData() {
    try {
      ...
      let res = await activityQuestionStatisticsApi.queryPage(queryForm);
  ...
  async function toDelete(record) {
    try {
      await activityQuestionStatisticsApi.delete(record.id);
