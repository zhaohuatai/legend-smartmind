  ...
  >
    ...
    <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
      <a-form-item label="姓名" name="perName">
        <a-input v-model:value.trim="form.perName" placeholder="请输入姓名" />
      </a-form-item>
      ...
      <a-form-item label="登录名" name="account">
        <a-input v-model:value.trim="form.account" placeholder="请输入登录名" />
...
<script setup>
  ...
  async function addEmployee(keepAdding) {
    try {
      ...
      emit('show-account', form.account, data);
      if (keepAdding) {
        reset();
      } else {
        onClose();
