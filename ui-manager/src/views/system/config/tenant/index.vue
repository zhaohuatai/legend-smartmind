  ...
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="租户" class="smart-query-form-item">
            <a-select v-model:value="queryForm.tenantId" placeholder="请选择租户" style="width: 200px" show-search :filter-option="filterOption" allowClear>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-row justify="end">
        <TableOperator class="smart-margin-bottom5" v-model="columns" :tableId="TABLE_ID_CONST.SYSTEM.CONFIG_TENANT" :refresh="queryData" />
      </a-row>
    ...
    <ConfigTenantFormModal ref="formModalRef" @reloadList="resetQuery" />
  </div>
</template>
...
<script setup>
  ...
  import ConfigTenantFormModal from './config-tenant-form-modal.vue';
  ...
  async function queryData() {
    try {
      ...
      let res = await configTenantApi.queryPage(queryForm);
  ...
  async function requestDelete(rowData) {
    SmartLoading.show();
    try {
      await configTenantApi.delete(rowData.id);
