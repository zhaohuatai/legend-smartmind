    ...
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        ...
        <a-form-item label="状态" class="smart-query-form-item">
          <SmartEnumSelect width="120px" enum-name="STATUS_ENUM" v-model:value="queryForm.status" />
        </a-form-item>
      ...
      >
        <template #bodyCell="{ text, record, column }">
          <template v-if="column.dataIndex === 'menuType'">
            <a-tag :color="menuTypeColor[text]">{{ $smartEnumPlugin.getDescByValue('MENU_TYPE_ENUM', text) }}</a-tag>
          </template>
          ...
          <template v-if="column.dataIndex === 'iconCls'">
            <component :is="$antIcons[text]" />
          </template>

          <template v-if="column.dataIndex === 'status'">
            <span>{{ $dictPlugin.getDataLabels('sys_normal_disable', text) }}</span>
          </template>

          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate">
              <a-button
                v-if="record.menuType !== MENU_TYPE_ENUM.POINTS.value && record.menuType !== MENU_TYPE_ENUM.MENU.value"
              ...
              <a-button v-if="record.parentId !== '-2'" v-privilege="'system:menu:update'" type="link" size="small" @click="showDrawer(record)">编辑</a-button>
              <a-button v-if="record.parentId !== '-2'" v-privilege="'system:menu:batchDelete'" danger type="link" @click="singleDelete(record)">删除</a-button>
            </div>
          </template>
        </template>
      </a-table>
...
<script setup>
  ...
  import { computed, createVNode, onMounted, reactive, ref, nextTick } from 'vue';
  ...
  // ------------------------ 表格渲染 ------------------------
  const menuTypeColor = {
    [MENU_TYPE_ENUM.CATALOG.value]: 'blue',
    [MENU_TYPE_ENUM.MENU.value]: 'green',
    [MENU_TYPE_ENUM.POINTS.value]: 'orange',
  };
  ...
  async function query() {
    try {
      ...
      let res = await menuApi.queryMenuList();
      if (queryForm.keywords || queryForm.menuType || queryForm.status || queryForm.hide) {
        tableData.value = buildMenuTableTree(filterMenuByQueryForm(res.data, queryForm));
      } else {
        tableData.value = buildMenuTableTree(res.data);
      }
  ...
  function confirmBatchDelete(menuArray) {
    const menuNameArray = menuArray.map((e) => e.name);
    Modal.confirm({
      title: '确定要删除如下菜单吗?',
