      ...
      <template #bodyCell="{ record, column }">
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            ...
            <a-button
              danger
              v-if="record.deptId !== topDepartmentId"
              v-privilege="'system:department:delete'"
              @click="deleteDepartment(record.deptId)"
              type="link"
              >删除</a-button
            >
          </div>
...
<script setup>
  ...
  async function queryDepartmentTree() {
    try {
      ...
      let res = await departmentApi.queryDeptTreeList();
      let data = res.data;
      ...
      if (!_.isEmpty(departmentTreeData.value) && departmentTreeData.value.length > 0) {
        topDepartmentId.value = departmentTreeData.value[0].deptId;
      }
