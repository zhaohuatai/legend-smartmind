...
import { buildRoutes, normalizeMenuList, router } from '/@/router';
...
async function getLoginInfo() {
  try {
    ...
    userInfo.menuList = normalizeMenuList(menuRes.data);

    const dictRes = await dictApi.getAllDictData();
    //构建系统的路由
    let menuRouterList = userInfo.menuList.filter((e) => e.path || e.frameUrl);
...
let csrfToken = localRead('CSRF_TOKEN');
if (!token && !csrfToken) {
  await initVue();
} else {
  await getLoginInfo();
